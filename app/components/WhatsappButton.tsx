...
interface WhatsAppButtonProps {
  phoneNumber?: string; // e.g. "94771234567" (country code without +)
  defaultMessage?: string;
}

export default function WhatsAppButton({
  phoneNumber = "94774298325",
  defaultMessage = "Hello, I would like to inquire about residential care at Senior Citizen Villa, Horana.",
}: WhatsAppButtonProps) {
        ...
        <div className="hidden sm:flex items-center gap-3 bg-[#0D1A12] border border-[#C5A059]/40 text-[#F7F4EE] px-4 py-3 rounded-lg shadow-2xl max-w-xs animate-fade-in relative">
          <div className="space-y-0.5">
            ...
            <p className="text-xs text-[#A3B0A7]">
              Need help? Chat with our Horana care team.
            </p>
          </div>
          <button
            onClick={() => setShowTooltip(false)}
            aria-label="Close message"
            className="text-[#A3B0A7] hover:text-white text-xs ml-1"
          >
            ✕
          </button>
